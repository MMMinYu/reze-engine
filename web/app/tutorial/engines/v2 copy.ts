...
import init, { WasmModel } from "../../../../reze-mmd/pkg/reze_mmd"
...
export class EngineV2 {
  ...
  private async loadModel() {
    ...
    // Load PMX file
    const response = await fetch("/models/塞尔凯特2/塞尔凯特2.pmx")
    if (!response.ok) {
    ...
    const wasmModel = new WasmModel(buffer)
    const vertices = wasmModel.get_vertices()
    const indices = wasmModel.get_indices()
    const textures = wasmModel.get_textures()
    const materials = wasmModel.get_materials()
    console.log(textures, materials)
    // Convert to typed arrays
    this.model = { vertices: new Float32Array(vertices), indices: new Uint32Array(indices) }
  }
